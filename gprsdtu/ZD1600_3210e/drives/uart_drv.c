...
u32	guart_rx_dp = 0;//guart rx dispose position
...
const u32 COM_POR_CLK[COMn] = 
{	
	ZD1600_COM1_GPIO_CLK,
	ZD1600_COM2_GPIO_CLK,
	ZD1600_COM3_GPIO_CLK
};

const u16 COM_TX_PIN[COMn] = 
{	
	ZD1600_COM1_TxPin, 
	ZD1600_COM2_TxPin, 
	ZD1600_COM3_TxPin 
...
/*usart configuration*/
const u16 COM_WordLength[2] = 
{	
	((u16)0x0000), 
	((u16)0x1000) 
};

const u16 COM_StopBits[4] = 
{	
	((u16)0x0000), 
	((u16)0x1000), 
	((u16)0x2000), 
...
{
	if(uart2_buff.rxlen >= guart_rx_dp) 
	{
	...
	else if(uart2_buff.rxlen < guart_rx_dp)
	{
		uart2_buff.rxbuf[uart2_buff.rxlen++] = USART_ReceiveData(ZD1600_COM2);
		if(uart2_buff.rxlen >= guart_rx_dp)
		{
