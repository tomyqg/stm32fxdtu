...
#define  OS_CPU_CM3_ST_RCC_CFGR	    (*((volatile INT32U *)0x40021004))   /* Clock Configer Value Reg. */
...
void  OS_CPU_SysTickInit (void)
{
	...
	cnts = OS_CPU_ClkFreq() / OS_TICKS_PER_SEC;
...
{   
    ...
    ccfg         = (INT32U)(OS_CPU_CM3_ST_RCC_CFGR);   
	switch (ccfg & 0x0C) {
	case 0x0:	  // SHI is System Clock Source.
		cpu_clk_freq =  CPU_OSC_FREQ; // 8MHz
		break;
	case 0x4:	  // SHE is System Clock Source.
		cpu_clk_freq = CPU_OSC_FREQ;
		break;
	case 0x8:	  // PLL is System Clock Source.
		cpu_clk_freq = CPU_OSC_FREQ;
		if (!((ccfg & 0x10000) && !(ccfg & 0x20000))) 
			cpu_clk_freq /= 2;
		tmp = (ccfg &(0xF<<18))>>18;
		cpu_clk_freq *= (tmp + 2) & 0xf;	
		break;
	default:
		return 0;
	}
