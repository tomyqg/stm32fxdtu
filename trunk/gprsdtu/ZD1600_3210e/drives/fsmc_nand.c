...
#define Bank_ATTR_ADDR     (Bank2_NAND_ADDR | 0x08000000)
...
void FSMC_NAND_Init(void)
{
  ...
  FSMC_NAND_PCCARDTimingInitTypeDef pAttr;
...
/* INT2 NAND pin configuration */  
  ...
  FSMC_NANDInitStructure.FSMC_AttributeSpaceTimingStruct = &pAttr;
...
u32 FSMC_NAND_WriteSmallPage(u8 *pBuffer, NAND_ADDRESS Address, u32 NumPageToWrite)
{
  ...
  while((NumPageToWrite != 0x00) && (addressstatus == NAND_VALID_ADDRESS) && (status == NAND_READY))
  {
    ...
    *(vu8 *)(Bank_ATTR_ADDR | ADDR_AREA) = ADDR_3rd_CYCLE(ROW_ADDRESS);
...
u32 FSMC_NAND_ReadSmallPage(u8 *pBuffer, NAND_ADDRESS Address, u32 NumPageToRead)
{
  ...
  {	   
    ...
    *(vu8 *)(Bank_ATTR_ADDR | ADDR_AREA) = ADDR_3rd_CYCLE(ROW_ADDRESS); 
    ...
    while (FSMC_GetFlagStatus(FSMC_Bank2_NAND, FSMC_FLAG_RisingEdge) == RESET);
